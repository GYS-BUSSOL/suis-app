...
const securityId = computed(() => props.securityId)
...
const fetchAccessLevel = async () => {
  try {
    const response = await $api(`/apps/security/list`, {
      method: 'GET',
    });
...
const dialogModelValueUpdate = () => {
  securityData.visitor_name = null,
  securityData.visitor_photo = null,
  securityData.purpose = null,
  securityData.non_exp = null,
  securityData.duration = null,
  securityData.acc_level = [];
...
watch(
  [() => securityId.value, () => typeDialog.value, () => props.fetchTrigger],
    ([newId,newType]) => {
      if (newType === "Add") {
  ...
  refVForm.value?.validate().then(({ valid }) => {
    try {
      if (valid) {
        ...
        emit("SecurityData", { mode, formData: { ...securityData }, dialogUpdate: dialogModelValueUpdate });
      } else {
        loadingBtn.value[0] = false
      }
    } catch (err) {
                ...
                <div class="d-flex gap-x-4 align-center">
                  <div>
                    ...
                    <div class="text-h4 font-weight-medium" v-if="!isLoading">
                      {{ typeDialog == 'Edit' ? 'Edit' : 'Create New' }} Security Barcode
                    </div>
                  </div>
                </div>
              </div>
                ...
                >
                  <VRow>
                    ...
                    <VCol cols="12" md="6" v-if="!isLoading">
                      <VCol>
                          <AppTextField
                            v-model="securityData.visitor_name"
                      ...
                      <VCol>
                          <VFileInput
                            ...
                            v-model="securityData.visitor_photo"
                    ...
                    <VCol cols="12" md="6" v-if="!isLoading">
                      <VCol>
                        ...
                        />
                        <small v-if="securityData.non_exp == false">Access will be expire according to Expired Date</small>
